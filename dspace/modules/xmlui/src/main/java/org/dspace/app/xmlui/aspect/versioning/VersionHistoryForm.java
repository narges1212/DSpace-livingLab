...
public class VersionHistoryForm extends AbstractDSpaceTransformer {
    ...
    private static final Message T_new_version = message("xmlui.aspect.versioning.VersioningNavigation.context_create_version");
    ...
    public void addBody(Body body) throws WingException, SQLException, AuthorizeException
    {
        ...
        if(versionHistory!=null)
        {
            ...
            // Uncomment this block to grant the user the ability to version his own documents
            //if(context.getCurrentUser()==this.getItem().getSubmitter()){
            //    Table table = main.addTable("newVersion", 1, 1);
            //    Row row = table.addRow();
            //    row.addCell().addXref(contextPath + "/item/version?itemID=" + this.getItem().getID(), T_new_version);
            //}
        ...
        // Uncomment this block to grant the user the ability to version his own documents
        //else if(context.getCurrentUser()==this.getItem().getSubmitter()){
        //    Division main = createMain(body);
        //    Table table = main.addTable("newVersion", 1, 1);
        //    Row row = table.addRow();
        //    row.addCell().addXref(contextPath + "/item/version?itemID=" + this.getItem().getID(), T_new_version);
    ...
    private void createTable(Division main, VersionHistory history, boolean isItemView, Item item) throws WingException, SQLException
    {
        ...
        if(history != null)
        {
            for(Version version : history.getVersions())
            {
                ...
                row.addCell().addXref(contextPath + "/handle/" + version.getItem().getHandle(), "[v" + version.getVersionNumber() + "]");
                ...
                // row.addCell().addContent(version.getVersionDate().toLocaleString());
