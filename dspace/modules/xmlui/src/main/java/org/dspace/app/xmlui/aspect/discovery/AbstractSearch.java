...
public abstract class AbstractSearch extends AbstractDSpaceTransformer implements CacheableProcessingComponent{
    ...
    private List<String> resultsHandleList = new ArrayList<String>();  ///
     ...
     * @throws SQLException 
     */
    public void performSearch(DSpaceObject scope) throws UIException, SearchServiceException, IOException, ParseException, SQLException {
        ...
        /*
         * This part is added for livingLabs Interleave
         */
    
        Boolean lilainterleave =  false;
        HashMap<String, String> lilaProp = new HashMap<String, String>();
        LiLaConnector llconnector = new LiLaConnector();
        lilaProp = llconnector.getLiLaConfigProp();
        int maxqrel = 100;
        this.resultsHandleList.clear();
        
        if(lilaProp!=null){
        	try{
        		if(lilaProp.get("livinglabsisset").equals("true")){
        		        for (DSpaceObject dsr : this.queryResults.getDspaceObjects()) {   
        		     		this.resultsHandleList.add("{'clicked': 'false','docid':'"+ dsr.getHandle() +"','team': 'siteRanking' }"); 
        		        } 
		        		if(lilaProp.get("interleaveisset").equals("true")){
		        			lilainterleave = true;
		        			if(lilaProp.get("lilaQrelMax")!=null) {
		        				maxqrel = Integer.parseInt(lilaProp.get("lilaQrelMax"));
		        			}
		
		        		}
        		} else {
        			lilaLog.info("Living Lab is deactive!");
        		}
        	} catch(Exception e){
	        	lilaLog.info("lila config properties error!");
	        }
        }

        if(lilainterleave){
        	try{
	        	if(llconnector.isLiLaQuery(queryArgs,scope)){
	        		 lilaLog.info("starting  livinglab ");
	      	         int ppage  =  queryArgs.getMaxResults();
	      	         int startf =  queryArgs.getStart();
	      	         String team;
	                 
	                 Map<String, List<String>> interleavedResult = llconnector.startLivingLabs(queryArgs, scope, context );
	                 
	                 List<String> teamSite,teamParticipant = null;
	    			 if(interleavedResult!=null) {
	    	            List<String> ivResult = interleavedResult.get("result");
	    	            teamParticipant = interleavedResult.get("teamB");
	    	            teamSite = interleavedResult.get("teamA");
	    	            this.queryResultsall =  llconnector.getLLQueryResults(queryArgs, scope, context, maxqrel);
	    	            lilaLog.info("max qrel is set to "+maxqrel);
	    	            lilaLog.info("size of interleaved result is "+ivResult.size());
	    	            int last  = ppage+startf;
	    	            if (last>ivResult.size()) 
	    	            	// if the interleaved result length is smaller than expected (unavailable items)
	    	            	last =  ivResult.size();
	    	            
	    	            for(int i=startf;i<last;i++){
	    	            	if (i-startf > this.queryResults.getDspaceObjects().size()) {
	    	            		break;
	    	            	}
	    	            	for(DSpaceObject dsp : this.queryResultsall.getDspaceObjects()) {  
	    	            		// to find the object with the handle string returned as interleaved result and replace it in the right position
	    	     	           	if(dsp.getHandle().equals(ivResult.get(i))){
	    	     	           		if((i-startf)<this.queryResults.getDspaceObjects().size()) {
	    	     	           			this.queryResults.getDspaceObjects().set(i-startf, dsp);
	    	     	           			break;
	    	     	           		}
	    	     	           	}
	    	                }
	    	            ...
	    	            lilaLog.info("interleave result: "+ivResult.toString());
	    	            lilaLog.info("start form: " + startf + "per page" + ppage);
	    	            
	    	            //for (DSpaceObject dsr : this.queryResults.getDspaceObjects()) {            //log just the current page  
	    	            for (String dsr : ivResult) { 
	    	            	if (teamParticipant.contains(dsr)){
	    	            		//team = "'participant'";
	    	            		team = interleavedResult.get("teams").get(0);
	    	            	} else if (teamSite.contains(dsr)) {
	    	            		team = "site";
	    	            	} else {
	    	            		team = "none";
	    	            	}
	    	         		this.resultsHandleList.add("{'clicked': 'false','docid':'"+ dsr +"','team':'"+team+"'}"); 
	    	            } 
	    			} else {
	    				lilaLog.info("interleaved result is null!!!");
	    			}
	        	}
	        }
	        catch(Exception e){
    ...
    protected void logSearch() {
        ...
        /*
         * This part is added for livingLabs Logger
         */
	   LiLaSearch initiater = new LiLaSearch();               ///
       LiLaSearchLogger responder = new LiLaSearchLogger();   ///
       initiater.addListener(responder);                      ///
       
       initiater.lilaLogAction(LogManager.getHeader(context, "search",", query( "+queryArgs.getQuery()+")"),dsoScope,
     		getParameterSortBy(),getParameterPage(),getParameterRpp(),getParameterOrder(),this.resultsHandleList);  
        /*
         * 
         * end of LivingLab part
         * */
